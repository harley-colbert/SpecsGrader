...
def test_training_xlsx_selects_quote_sheet_and_row5():
    ...
    rows.append(["", "", "", "Spec text XLSX", "Risk text XLSX", "medium", "electrical"])
    ...
    assert row["spec_text"] == "Spec text XLSX"
    assert row["specific_risk_existing"] == "Risk text XLSX"
...
def test_training_csv_reads_columns_and_validates_labels(tmp_path):
    ...
    assert dataset["summary"]["missing_risk_text"] == 0
...
def test_classify_loader_handles_csv_and_xlsx():
    ...
    rows = [["", "", "", "", "", "", ""] for _ in range(4)]
    rows.append(["", "", "", "Spec text", "Classify XLSX Text", "", ""])
